...
- Never commit `.env` files or credentials; never place secrets in client-visible code.
- Verify a package exists and is maintained before installing anything; ask before adding dependencies.
- Run lint and build before commit/push.
- Lint and build are not proof of completion: open the changed routes in the running app (including a mobile viewport) before reporting done.
